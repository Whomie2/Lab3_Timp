...
SUITE(KeyTest)
{
    ...
    TEST(Key_with_a_space) {
        CHECK_THROW(modAlphaCipher test(L"новый год"), cipher_error);
    }
    TEST(Key_with_invalid_characters_number) {
        CHECK_THROW(modAlphaCipher test(L"21саваг"), cipher_error);
    }
...
SUITE(EncryptTest)
{
    ...
    TEST_FIXTURE(KeyAB_fixture,NumberInText ) {
        wstring open_text = L"новый1234год";
...
SUITE(DecryptTest)
{
   TEST_FIXTURE(KeyAB_fixture, LargeLetters) {
    ...
    TEST_FIXTURE(KeyAB_fixture, EmptyText) {
        wstring cipher_text = L"33коровы";
